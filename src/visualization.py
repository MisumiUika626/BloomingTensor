...
def plot_binary_decision_region(model, dataset, save_path, title):
    """Plot raw model outputs and the 0.5 inspection boundary for 2-D data."""
    ...
    if samples.ndim != 2 or samples.shape[1] != 2:
        raise ValueError("decision-region plotting requires two input features")
    ...
    axes.scatter(
        ...
        vmax=1.0,
        edgecolors="black",
        s=100 if len(dataset) <= 20 else 30,
    )
    ...
    figure.colorbar(color_map, ax=axes, label="Raw model output")
    figure.tight_layout()
